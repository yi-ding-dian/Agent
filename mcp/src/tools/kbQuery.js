 ...
 * 知识库查询工具（kb_query）
 *
 * 供 MyAgent 内置工具注册（mcp/src/tools/kbQuery.js），使用方式：
 ...
 *   AGENT_CONFIG_BASE_URL 环境变量可选（默认 http://127.0.0.1:7980，与后端默认 port 一致），
 *   或进程内直接提供 process.env.KB_QUERY_LINK（两者都无则报错提示）。
 */

const DEFAULT_CONFIG_BASE = "http://127.0.0.1:7980";
